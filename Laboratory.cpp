...
Laboratory::Laboratory(char * no_tutorial, int & area, int * s_width, int * s_height, char * browser_path) :area(area)
{
    //ctor
    ...
    arrow = new Object(-0.0024+place, -0.0018, 0.0, //koordinatak
                       ...
                       true, "Textures/arrow.bmp", 0, "rectangle", 3, 0);

    map1 = new Object(0.0008, 0.0001, 0.0, //koordinatak
                      0, 0, 0, //elforgatas
                      0.006, 1.0, 0.009, //atmeretezes
                      true, "Textures/map1.bmp", 0, "rectangle", 3, 0);
...
void Laboratory::lab_events(int & area)
{
...
void Laboratory::lab_rendering(int & area)
{
    for (int i=0; i<6; i++)
    {
        if (selected[i] == 0)
    ...
    for (int i=0; i<obj_sum[area_selected]; i++)
